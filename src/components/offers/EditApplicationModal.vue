  ...
  >
    <div class="flex flex-col gap-6">
      ...
      <BaseInput
        ...
        label="URL do Logo"
        placeholder="Digite a URL do logo (opcional)"
        :disabled="loading"
        variant="dark"
      >
        <template #prefix>
      ...
      <BaseInput
        v-model="formData.base_affiliate_link"
        label="Link de Afiliado Base"
        placeholder="Digite o link de afiliado base"
        :disabled="loading"
        variant="dark"
      >
        <template #prefix>
  ...
  setup(props, { emit }) {
    ...
    const loadApplicationData = async () => {
      try {
        ...
        toast.error('Erro ao carregar dados da aplicação')
        emit('close')
      } finally {
        loading.value = false
      }
